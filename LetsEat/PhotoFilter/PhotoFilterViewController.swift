...
private extension PhotoFilterViewController {
    ...
    func saveSelectedPhoto(){
        if let mainImage = self.photoImageView.image{
            ...
            if let selResID = selectedRestaurantID{
                restPhotoItem.restaurantID = Int64(selResID)
            }
...
extension PhotoFilterViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let selectedImage = info[UIImagePickerController.InfoKey.editedImage] as? UIImage {
            ...
            let mainImageViewSize = photoImageView.frame.size
...
extension PhotoFilterViewController: ImageFiltering {
    func filterMainImage(filterItem: FilterItem) {
        if let mainImage = mainImage, let filter = filterItem.filter {
            if filter != "None" {
                photoImageView.image = self.apply(filter: filter, originalImage: mainImage)
            } else {
                photoImageView.image = mainImage
            }
        }
